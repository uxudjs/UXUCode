...
| 特性 | 說明 |
|---|---|
| Complete engineering flow | 涵蓋需求、規格、規劃、實作、測試、審查、簡化、遷移和發布 |
| Specification driven | 非簡單任務先建立可驗證規格 |
| Incremental implementation | 按垂直切片完成，每個任務獨立驗證 |
| Minimal correct implementation | 優先重用現有程式碼、標準函式庫和平台能力 |
| Verification first | 完成必須提供測試、建置或執行證據 |
| Multidimensional review | 檢查正確性、可讀性、架構、安全、效能和複雜度 |
| Concise communication | 刪除重複內容，同時保留必要技術資訊 |
| Release gate | 輸出 Blocker、Recommended、Acknowledged 和 GO／NO-GO |
| Context compression | 精確保留程式碼、命令、路徑、連結與結構 |
| Consistent dual CLI | 兩端適配獨立，命令與工作流語意一致 |
...
**兩端能共用同一安裝目錄嗎？ 不需要。安裝對應宿主目錄即可，兩端套件完整且獨立。?**兩端能共用同一安裝目錄嗎？ 不需要。安裝對應宿主目錄即可，兩端套件完整且獨立。

**為什麼 plan 不直接編碼？ 它只把已核准規格拆成可驗證任務。?**為什麼 plan 不直接編碼？ 它只把已核准規格拆成可驗證任務。

**什麼時候使用 ship？ 功能完成並準備合併、打版本或發布時；存在 Blocker 或關鍵證據缺失時必須 NO-GO。?**什麼時候使用 ship？ 功能完成並準備合併、打版本或發布時；存在 Blocker 或關鍵證據缺失時必須 NO-GO。

**ultra 會省略安全風險嗎？ 不會，正確性、安全、證據和明確要求永遠高於簡潔度。?**ultra 會省略安全風險嗎？ 不會，正確性、安全、證據和明確要求永遠高於簡潔度。
