...
description: 为代码添加埋点，使生产行为可见可诊断。适用于添加日志、指标、链路追踪或告警时。适用于发布任何在生产环境中运行的功能且需要证明其正常工作时。适用于生产环境问题已上报但现有数据无法定位原因时。
---
...
For the at-a-glance version of this list, including the pre-launch instrumentation gate, see the [observability checklist](../../references/observability-checklist.md).
