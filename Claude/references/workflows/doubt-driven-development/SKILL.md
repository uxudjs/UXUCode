...
This is not `/uxu-code:review`. That command is a verdict on a finished artifact. Doubt-driven development is an in-flight check while course correction is still cheap.
...
This workflow belongs to the main-session orchestrator. Claude may select the host-native `reviewer`, `security-reviewer`, or `test-reviewer` role that best matches the decision. Use one role per ordinary cycle; add specialized review only when the actual risk requires it.
...
- `code-review-and-quality` and `/uxu-code:review`: use them for the final artifact; doubt-driven development checks non-trivial decisions while work is still in progress.
