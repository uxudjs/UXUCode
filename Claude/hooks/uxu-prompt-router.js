...
process.stdin.on('end', () => {
  ...
  if (!prompt.toLowerCase().startsWith('/uxu-code:')) return;

  const [firstLine, ...bodyLines] = prompt.split(/\r?\n/);
  const match = firstLine.match(/^\/uxu-code:([a-z-]+)(?:[ \t]+(.*))?$/);
  if (!match) {
    reject('UXUCode rejected invalid command format. Use /uxu-code:<command> or /uxu-code:help.');
    return;
  }
  ...
  if (!commands.has(command)) {
    reject(`UXUCode rejected unknown command "${command}". Use /uxu-code:help.`);
    return;
  }
  ...
  if (command === 'status') {
    ...
    emit(`Route this request to the "status" skill with arguments "". Canonical UXUCode status payload:\n${snapshot.line}\nCurrent task: ${snapshot.currentTask}\nLast update: ${snapshot.updatedAt}\nUse the status line exactly as the first line; do not rerun or reconstruct it. ${policyFor(mode)} ${workflowPolicy}`);
    return;
  }
