...
description: 进行多维度代码审查。适用于合并任何变更之前。适用于审查自己、其他 agent 或人类编写的代码。适用于代码合入主分支前需要从多维度评估代码质量时。
---
...
If reviewer delegation fails, is unavailable, lacks fresh-context isolation, or returns no usable result, mark that role's review incomplete and report the failure. Do not replace it with an external model CLI, manual external copying, or self-review presented as independent validation.
...
- For detailed security review guidance, see the [security checklist](../../references/security-checklist.md)
- For performance review checks, see the [performance checklist](../../references/performance-checklist.md)
