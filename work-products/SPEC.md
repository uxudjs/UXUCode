...
默认把 UXUCode 新建的测试放入 `work-products/tests/`，但每个测试必须满足至少一项：

- 被仓库统一验证入口显式执行；
- 已加入项目现有测试框架的发现配置；
- 在计划和最终验证证据中记录完整可运行命令。

如果框架、CI、共置测试约定或发布打包要求必须使用项目原生测试目录，则项目约定优先。该例外必须在计划依据和最终验证中说明，不能静默偏离。

不得自动移动使用者仓库已有测试。UXUCode 本仓库的合同测试可在本次实现中统一迁入 `work-products/tests/`；`OpenClaw/tests/` 属于独立交付模块的原生测试，不在本次迁移范围。
...
- 不移动使用者仓库已有的项目原生测试，也不修改未知测试框架配置。
...
- **过程目录规则污染产品结构：** 项目原生测试/打包约束拥有显式例外。
...
- 项目源码、项目原生测试、交付文件和无法确认归属的文件不被移动。
...
只有存在充分证据属于 UXUCode 过程产物的文件才可进入迁移集合：
...
- 项目原生测试及其 fixture、snapshot 或测试框架配置；
- 仅凭文件名、扩展名、Git 未跟踪状态或位于 `tests/`、`tasks/` 等通用目录而无法确认归属的文件；
...
存在目标冲突、路径逃逸、无法读取的候选文件或无法安全重写的有效引用时，状态必须为 `BLOCKED`；此时即使随后调用 `apply` 也不得部分执行。
...
- 对引用迁移源路径的仓库内文本文件，将精确旧路径改为新路径；
- 对迁移集合中文件之间的引用，使用完整映射一次性计算最终路径，禁止按移动顺序逐步猜测；
- 外部 URL、绝对路径、无法解析的动态路径、自然语言示例和不指向真实迁移对象的同名文本不得改写。
...
- `apply` 将已确认的 SPEC、task 和 UXUCode 辅助测试移动到正确位置；
- 移动文件自身的相对链接、迁移集合内部引用和外部文件的精确旧路径引用均正确更新；
- 项目原生测试、源码、交付文件和模糊候选保持逐字节不变；
- 目标冲突、路径逃逸、符号链接、不可读文件和歧义引用导致 `BLOCKED` 且零部分写入；
...
- `apply` 仅移动可证明属于 UXUCode 的错放过程产物，并完成必要引用改写。
- 无法确认的文件不移动；项目原生测试与源码保持不变。
